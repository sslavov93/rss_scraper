...
class TestGetUnreadItems(BaseTestFixture):
    def test_get_unread_items_from_feed_no_auth(self):
        response = self.client.get('/api/my-feeds/1/new')
        self.assertEqual(401, response.status_code)

    def test_get_unread_items_from_feed_when_feed_is_missing(self):
    ...
    def test_get_unread_items_from_feed_if_not_followed(self):
        response = self.client.get(
            '/api/my-feeds/1/new',
    ...
    def test_get_unread_items_from_feed_no_error(self):
        response = self.client.get(
            '/api/my-feeds/1/new',
            headers=basic_auth_headers("user", "pass")
        )
    ...
    def test_get_all_unread_items_when_user_follows_no_feed(self):
        response = self.client.get(
            '/api/my-feeds/new',
            headers=basic_auth_headers("user3", "pass")
        )
    ...
    def test_get_all_unread_items_when_user_follows_some_feeds(self):
        response = self.client.get(
            '/api/my-feeds/new',
            headers=basic_auth_headers("user", "pass")
        )
